...
import { Servers, Users, JoinedServer, UserPlaylist } from "./dbObject.js";
import { T_DATA, T_UserPlaylist } from "./types.js";
...
export async function RegisterUser(interaction: Interaction) {
  ...
  if (!(await Servers.findOne({ where: { id: interaction.guildId } }))) {
    ...
    console.log(
      `${interaction.guild?.name} 서버가 데이터베이스에 등록되었습니다.`
    );
  }
  // User 등록
  if (!(await Users.findOne({ where: { id: interaction.user.id } }))) {
    ...
    console.log(
      `${interaction.user.username}님께서 데이터베이스에 등록되었습니다.`
    );
  }
  // JoinedServer 등록
  if (
    !(await JoinedServer.findOne({
      where: { server_id: interaction.guildId, user_id: interaction.user.id },
    }))
  ) {
    await JoinedServer.create({
      server_id: interaction.guildId,
      user_id: interaction.user.id,
    });
    console.log(
      `${interaction.user.username}님께서 ${interaction.guild?.name} 서버에 등록되었습니다.`
    );
  }
...
export async function RegisterUserMsg(interaction: Message) {
  ...
  if (!(await Users.findOne({ where: { id: interaction.author.id } }))) {
    ...
    console.log(
      `${interaction.author.username}님께서 데이터베이스에 등록되었습니다.`
    );
  }
  // JoinedServer 등록
  if (
    ...
    console.log(
      `${interaction.author.username}님께서 ${interaction.guild?.name} 서버에 등록되었습니다.`
    );
  }
...
export async function GetUserPlaylist(playlist: T_DATA): Promise<T_UserPlaylist> {
  const userPlaylist: T_UserPlaylist = {
    ...
    playlist: playlist.dataValues.playlist,
  };
